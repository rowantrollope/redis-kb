...
url: https://redis.io/docs/latest/operate/rs/networking/port-configurations/
retrieved_utc: '2026-04-09T20:46:05.849325+00:00'
tags:
- official
- docs
- sitemap
fetched_url: https://redis.io/docs/latest/operate/rs/networking/port-configurations/index.html.md
---
...
|----------|------|--------------|-------------------|-------------|
| TCP | 8001 | <span title="Not configurable">&#x274c; No</span> | Internal, External | Traffic from application to Redis SoftwareÂ [Discovery Service]() |
...
```sh
rladmin cluster config cm_portÂ <custom-port>
```
...
```sh
$ rladmin cluster config envoy_admin_portÂ <new-port>
...
```sh
$ rladmin cluster config envoy_mgmt_server_portÂ <new-port>
...
```sh
$ rladmin cluster config gossip_envoy_admin_portÂ <new-port>
Cluster configured successfully
```
...
```sh
rladmin cluster config cnm_http_portÂ <new-port>
```

```sh
rladmin cluster config cnm_https_portÂ <new-port>
```
