...
url: https://redis.io/docs/latest/develop/ai/redisvl/api/searchindex/
retrieved_utc: '2026-04-09T20:46:07.324445+00:00'
tags:
- official
- docs
- sitemap
fetched_url: https://redis.io/docs/latest/develop/ai/redisvl/api/searchindex/index.html.md
---
...
```json metadata
{
  ...
  "description": "",
  "categories": null,
  "tableOfContents": {"sections":[{"children":[{"id":"class-searchindexschema-redisclientnone-redisurlnone-connectionkwargsnone-validateonloadfalse-kwargs","title":"class SearchIndex(schema, redisclient=None, redisurl=None, connectionkwargs=None, validateon_load=False, **kwargs)"}],"id":"searchindex","title":"SearchIndex"},{"children":[{"id":"class-asyncsearchindexschema-redisurlnone-redisclientnone-connectionkwargsnone-validateonloadfalse-kwargs","title":"class AsyncSearchIndex(schema, , redisurl=None, redisclient=None, connectionkwargs=None, validateon_load=False, *kwargs)"}],"id":"asyncsearchindex","title":"AsyncSearchIndex"}]}

,
  "codeExamples": [{"id":"searchindex-api"},{"id":"asyncsearchindex-api"}]
}
```
...
* **Parameters:**
  * **schema** ([*IndexSchema*]()) â Index schema object.
  * **redis_client** (*Optional* *[* *Redis* *]*) â An
    instantiated redis client.
  * **redis_url** (*Optional* *[* *str* *]*) â The URL of the Redis server to
    connect to.
  * **connection_kwargs** (*Dict* *[* *str* *,* *Any* *]* *,* *optional*) â Redis client connection
    args.
  * **validate_on_load** (*bool* *,* *optional*) â Whether to validate data against schema
...
* **Parameters:**
  * **queries** (*List* *[* *SearchParams* *]*) â The queries to search for.
  * **batch_size** (*int* *,* *optional*) â The number of queries to search for at a time.
    Defaults to 10.
* **Returns:**
...
here, we canât easily give control of the keys weâre clearing to the
...
* **Parameters:**
  **redis_url** (*Optional* *[* *str* *]* *,* *optional*) â The URL of the Redis server to
  connect to.
* **Raises:**
  * **redis.exceptions.ConnectionError** â If the connection to the Redis
        server fails.
  * **ValueError** â If the Redis URL is not provided nor accessible
        through the REDIS_URL environment variable.
  * **ModuleNotFoundError** â If required Redis modules are not installed.
...
* **Parameters:**
  * **overwrite** (*bool* *,* *optional*) â Whether to overwrite the index if it
    already exists. Defaults to False.
  * **drop** (*bool* *,* *optional*) â Whether to drop all keys associated with the
    index in the case of overwriting. Defaults to False.
* **Raises:**
  * **RuntimeError** â If the index already exists and âoverwriteâ is False.
  * **ValueError** â If no fields are defined for the index.
* **Return type:**
  None
...
* **Parameters:**
  **drop** (*bool* *,* *optional*) â Delete the key / documents pairs in the
  index. Defaults to True.
* **Raises:**
  **redis.exceptions.ResponseError** â If the index does not exist.
...
the indexâs key prefix and separator configuration.
...
* **Parameters:**
  **ids** (*Union* *[* *str* *,* *List* *[* *str* *]* *]*) â The document ID or IDs to remove from the index.
* **Returns:**
...
Remove a specific entry or entries from the index by itâs key ID.

* **Parameters:**
  **keys** (*Union* *[* *str* *,* *List* *[* *str* *]* *]*) â The document ID or IDs to remove from the index.
* **Returns:**
  Count of records deleted from Redis.
* **Return type:**
  int
...
* **Parameters:**
  * **keys** (*Union* *[* *str* *,* *List* *[* *str* *]* *]*) â The entry ID or IDs to set the expiration for.
  * **ttl** (*int*) â The time-to-live in seconds.
* **Return type:**
  int | *List*[int]
...
* **Parameters:**
  **id** (*str*) â The specified unique identifier for a particular
...
* **Parameters:**
  **schema_dict** (*Dict* *[* *str* *,* *Any* *]*) â A dictionary containing the schema.
* **Returns:**
...
* **Parameters:**
  * **name** (*str*) â Name of the search index in Redis.
  * **redis_client** (*Optional* *[* *Redis* *]*) â An
    instantiated redis client.
  * **redis_url** (*Optional* *[* *str* *]*) â The URL of the Redis server to
    connect to.
* **Raises:**
  **ValueError** â If redis_url or redis_client is not provided.
...
* **Parameters:**
  **schema_path** (*str*) â Path to the YAML schema file.
* **Returns:**
  A RedisVL SearchIndex object.
* **Return type:**
  [SearchIndex](#searchindex)
...
* **Parameters:**
  **name** (*str* *,* *optional*) â Index name to fetch info about.
  Defaults to None.
* **Returns:**
...
* **Parameters:**
  * **data** (*Iterable* *[* *Any* *]*) â An iterable of objects to store.
  * **id_field** (*Optional* *[* *str* *]* *,* *optional*) â Specified field used as the id
    portion of the redis key (after the prefix) for each
    object. Defaults to None.
  * **keys** (*Optional* *[* *Iterable* *[* *str* *]* *]* *,* *optional*) â Optional iterable of keys.
    Must match the length of objects if provided. Defaults to None.
  * **ttl** (*Optional* *[* *int* *]* *,* *optional*) â Time-to-live in seconds for each key.
    Defaults to None.
  * **preprocess** (*Optional* *[* *Callable* *]* *,* *optional*) â A function to preprocess
    objects before storage. Defaults to None.
  * **batch_size** (*Optional* *[* *int* *]* *,* *optional*) â Number of objects to write in
    a single Redis pipeline execution. Defaults to classâs
    default batch size.
* **Returns:**
...
* **Return type:**
  List[str]
* **Raises:**
  * **SchemaValidationError** â If validation fails when validate_on_load is enabled.
  * **RedisVLError** â If thereâs an error loading data to Redis.
...
* **Parameters:**
  * **query** (*BaseQuery*) â The search query to be executed.
  * **page_size** (*int* *,* *optional*) â The number of results to return in each
    batch. Defaults to 30.
* **Yields:**
  A generator yielding batches of search results.
* **Raises:**
  * **TypeError** â If the page_size argument is not of type int.
  * **ValueError** â If the page_size argument is less than or equal to zero.
* **Return type:**
  *Generator*
...
* **Parameters:**
  **query** (*Union* *[* *BaseQuery* *,* *AggregateQuery* *]*) â The query to run.
* **Returns:**
...
* **Parameters:**
  **redis_client** (*Redis*) â A Redis or Async Redis
  client instance to be used for the connection.
* **Raises:**
  **TypeError** â If the provided client is not valid.
...
* **Parameters:**
  ...
  * **redis_url** (*Optional* *[* *str* *]* *,* *optional*) â The URL of the Redis server to
    connect to.
  * **redis_client** (*Optional* *[* *AsyncRedis* *]*) â An
    instantiated redis client.
  * **connection_kwargs** (*Optional* *[* *Dict* *[* *str* *,* *Any* *]* *]*) â Redis client connection
    args.
  * **validate_on_load** (*bool* *,* *optional*) â Whether to validate data against schema
    when loading. Defaults to False.
...
* **Parameters:**
  * **queries** (*List* *[* *SearchParams* *]*) â A list of search queries to execute.
    Each query can be either a string or a tuple of (query, params).
  * **batch_size** (*int* *,* *optional*) â The number of queries to execute in each
...
we canât easily give control of the keys weâre clearing to the user so they
...
* **Parameters:**
  **drop** (*bool* *,* *optional*) â Delete the documents in the index.
  Defaults to True.
* **Raises:**
  **redis.exceptions.ResponseError** â If the index does not exist.
...
* **Parameters:**
  ...
  * **ttl** (*Optional* *[* *int* *]* *,* *optional*) â Time-to-live in seconds for each key.
    Defaults to None.
  * **preprocess** (*Optional* *[* *Callable* *]* *,* *optional*) â A function to
