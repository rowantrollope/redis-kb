...
retrieved_utc: '2026-04-09T20:45:53.280939+00:00'
tags:
- official
- docs
- sitemap
fetched_url: https://redis.io/docs/latest/integrate/write-behind/quickstart/write-behind-guide/index.html.md
---
...
```json metadata
{
  ...
  "group": "di",
  "tableOfContents": {"sections":[{"id":"concepts","title":"Concepts"},{"id":"supported-data-stores","title":"Supported data stores"},{"id":"prerequisites","title":"Prerequisites"},{"children":[{"id":"write-behind-jobs","title":"Write-behind jobs"},{"id":"source-section","title":"Source section"},{"id":"output-section","title":"Output section"},{"id":"apply-filters-and-transformations-to-write-behind","title":"Apply filters and transformations to write-behind"}],"id":"preparing-the-write-behind-pipeline","title":"Preparing the write-behind pipeline"},{"id":"provide-targets-secrets","title":"Provide target's secrets"},{"id":"deploy-the-write-behind-pipeline","title":"Deploy the write-behind pipeline"},{"id":"monitor-the-write-behind-pipeline","title":"Monitor the write-behind pipeline"},{"id":"upgrading","title":"Upgrading"}]}
...
- The `row_format` attribute can be used with the value `full` to receive both the `before` and `after` sections of the payload. Note that for write-behind events the `before` value of the key is never provided.
 ...
 > Note: Write-behind does not support the [`expired`](#events-generated-by-different-commands) event. Therefore, keys that are expired in Redis will not be deleted from the target database automatically.
...
| Created incoming events by stream   | `rdi_metrics_incoming_entries{data_source:"â¦",operation="inserted"}`                                |
| Updated incoming events by stream   | `rdi_metrics_incoming_entries{data_source:"â¦",operation="updated"}`                                 |
| Deleted incoming events by stream   | `rdi_metrics_incoming_entries{data_source:"â¦",operation="deleted"}`                                 |
| Filtered incoming events by stream  | `rdi_metrics_incoming_entries{data_source:"â¦",operation="filtered"}`                                |
| Malformed incoming events by stream | `rdi_metrics_incoming_entries{data_source:"â¦",operation="rejected"}`                                |
| Total events per stream (snapshot)  | `rdi_metrics_stream_size{data_source:""}`                                                           |
| Time in stream (snapshot)           | `rdi_metrics_stream_last_latency_ms{data_source:"â¦"}`                                               |
