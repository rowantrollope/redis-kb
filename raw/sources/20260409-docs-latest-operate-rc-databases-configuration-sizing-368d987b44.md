...
url: https://redis.io/docs/latest/operate/rc/databases/configuration/sizing/
retrieved_utc: '2026-04-09T20:46:07.487586+00:00'
tags:
- official
- docs
- sitemap
fetched_url: https://redis.io/docs/latest/operate/rc/databases/configuration/sizing/index.html.md
---
...
```json metadata
{
  ...
  "tableOfContents": {"sections":[{"id":"dataset-size-dataset-size","title":"Dataset size {#dataset-size}"},{"children":[{"id":"throughput-factors-and-variability","title":"Throughput factors and variability"},{"id":"optimize-throughput","title":"Optimize throughput"},{"id":"frequently-asked-questions","title":"Frequently asked questions"}],"id":"throughput","title":"Throughput"}]}
...
- [Replication]() doubles memory consumption; that is, 512 MB of data requires at least 1 GB of memory limit when replication is enabled. This affects both Redis Cloud Pro and Redis Cloud Essentials. For example, if you subscribe to a 1 GB Essentials plan, Redis will allocate 512 MB for your dataset and the other 512 MB for replication.

- [Active-Active]() also doubles memory consumption and the effect is cumulative with the impact of replication. Since Active-Active requires replication to be turned on, the memory limit impact can be as large as four times (4x) the original data size.

- [Advanced capabilities]() also consume memory. For search databases, consider index size when you size your database. See [Search and query sizing]() for more info.
...
- **Command complexity**: Simple commands, like `GET` and `SET`, are faster and require fewer resources, whereas more complex commands involve more processing time and can reduce throughput. See the [Command list]() to see which commands are more complex than others.
- **Replication**: Using [multi-zone replication]() affects throughput as each write operation is executed asynchronously in each zone.
- **Security**: Some security options, such as [transport layer security](), may affect throughput.
...
- Track the slow logs using the [`SLOWLOG` command]() or the **Slowlog** tab on the [database screen]().
- Use [pipelining]() and [concurrent connections]() effectively to optimize throughput and latency.
- Search databases have their own throughput requirements. See [Search and query sizing]() for more info.
...
Factors like high connection counts, complex commands, large payloads, and network limitations can affect throughput. Test and adjust based on your applicationâs needs. 

**How do I know the optimal throughput for my application?**

Start with the expected ops/sec. Most of the time, itâs more than enough. For an average request size of less than 3KB, we suggest specifying lower ops/sec than expected to reduce costs. For request sizes higher than 3 KB, we suggest specifying higher ops/sec than your target. Benchmarking your application under production-like conditions will help you find the best configuration.
