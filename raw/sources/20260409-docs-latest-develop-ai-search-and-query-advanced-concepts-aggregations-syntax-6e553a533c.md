...
url: https://redis.io/docs/latest/develop/ai/search-and-query/advanced-concepts/aggregations-syntax/
retrieved_utc: '2026-04-09T20:45:52.788218+00:00'
tags:
- official
- docs
- sitemap
fetched_url: https://redis.io/docs/latest/develop/ai/search-and-query/advanced-concepts/aggregations-syntax/index.html.md
---
...
```json metadata
{
  ...
  "tableOfContents": {"sections":[{"id":"overview","title":"Overview"},{"id":"syntax-and-expression-ordering","title":"Syntax and expression ordering"},{"id":"when-to-use","title":"When to use @"},{"id":"groupby-with-multiple-reduce-operations","title":"GROUPBY with multiple REDUCE operations"},{"id":"multiple-apply-operations-followed-by-groupby-and-reduce","title":"Multiple APPLY operations followed by GROUPBY and REDUCE"},{"id":"filter-and-params","title":"FILTER and PARAMS"},{"id":"placement-of-filter-before-and-after-groupbyapply","title":"Placement of FILTER before and after GROUPBY/APPLY"},{"id":"load-after-groupbyreduce","title":"LOAD after GROUPBY/REDUCE"},{"id":"placement-rules-for-specific-parameters","title":"Placement rules for specific parameters"},{"id":"limit-and-withcursor-used-together","title":"LIMIT and WITHCURSOR used together"}]}
...
1. `query` â your query, which must be the second argument.
1. `FILTER` â filters raw documents before transformations or aggregation.
1. `LOAD` â loads document fields.
1. `APPLY` â applies transformations on fields.
1. `GROUPBY` â groups results by specific fields.
1. `REDUCE` â performs aggregations. For example, `SUM`, `COUNT`, and `AVG`.
1. `SORTBY` â orders the results based on specified fields.
1. `LIMIT` â restricts the number of results returned.
