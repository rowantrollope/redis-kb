...
url: https://redis.io/docs/latest/operate/rc/billing-and-payments/cost-report/
retrieved_utc: '2026-04-09T20:45:57.003110+00:00'
tags:
- official
- docs
- sitemap
fetched_url: https://redis.io/docs/latest/operate/rc/billing-and-payments/cost-report/index.html.md
---
...
```json metadata
{
  ...
  "tableOfContents": {"sections":[{"children":[{"id":"essentials-subscriptions","title":"Essentials subscriptions"},{"id":"pro-subscriptions","title":"Pro subscriptions"}],"id":"structure","title":"Structure"},{"id":"get-focus-format-using-rest-api","title":"Get FOCUS format using REST API"}]}
...
| **Plan size** | The size of the [subscription plan](), in MB |
| **Plan cost** | The cost of the subscription plan for the usage period |

All columns after the **Plan cost** column show the key values for any [tags]() you added to databases in all subscriptions. If a database has a tag with that key, the cell shows the tag value. You can use this information to filter and analyze your costs by tag.
...
| **High availability** | Whether the subscription is [highly available]() |
...
All columns after the **Total cost** column show the key values for any [tags]() you added to databases in all subscriptions. If a database has a tag with that key, the column shows the value for that tag. You can use this information to filter and analyze your costs by tag.
...
You can use the [Redis Cloud REST API]() to get a cost report in [FinOps Open Cost and Usage Specification (FOCUS)](https://focus.finops.org/) compatible format.



The cost report returned from the Redis Cloud REST API contains fields from the [FOCUS column library](https://focus.finops.org/focus-columns/). See [Generate FOCUS-compliant cost report with REST API]() for more details.