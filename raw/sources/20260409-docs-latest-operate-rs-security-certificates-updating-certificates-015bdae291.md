...
url: https://redis.io/docs/latest/operate/rs/security/certificates/updating-certificates/
retrieved_utc: '2026-04-09T20:45:55.761638+00:00'
tags:
- official
- docs
- sitemap
fetched_url: https://redis.io/docs/latest/operate/rs/security/certificates/updating-certificates/index.html.md
---
...
```json metadata
{
  ...
  "tableOfContents": {"sections":[{"id":"how-to-update-certificates","title":"How to update certificates"},{"children":[{"id":"update-proxy-certificates-update-ap-proxy-certs","title":"Update proxy certificates {#update-ap-proxy-certs}"}],"id":"replica-of-database-certificates","title":"Replica Of database certificates"},{"children":[{"id":"update-proxy-certificates-update-aa-proxy-certs","title":"Update proxy certificates {#update-aa-proxy-certs}"},{"id":"update-syncer-certificates-update-aa-syncer-certs","title":"Update syncer certificates {#update-aa-syncer-certs}"}],"id":"active-active-database-certificates","title":"Active-Active database certificates"},{"id":"troubleshoot-rhel-8-crypto-policy-and-certificate-key-size","title":"Troubleshoot RHEL 8 crypto policy and certificate key size"}]}
...
1. From the Cluster Manager UI, update the destination database (_replica_) configuration with the [new certificate]().
...
1. Use the [`crdb-cli`]() utility to update Active-Active database configuration from the command line. Run the following command once for each Active-Active database residing on the modified clusters:
...
1. Update the Active-Active database configuration from the command line with the [`crdb-cli`]() utility. Run this command once for each Active-Active database that resides on the modified clusters:
...
- Run step 2 as quickly as possible after step 1. Between the two steps, new syncer connections that use the âoldâ certificate will get rejected by the cluster that has been updated with the new certificate (in step 1).<br/>
