---
title: "Redis Sentinel official docs"
url: "https://redis.io/docs/latest/operate/oss_and_stack/management/sentinel/"
retrieved_utc: "2026-04-09T20:34:18.418284+00:00"
tags: ['ad-hoc']
---
...
- Book a meeting
- Login
- Redis Software latest ▼ ▲ latest v7.22 v7.8 v7.4 Redis Cloud Redis for Kubernetes latest ▼ ▲ latest v7.22 v7.8.6 v7.8.4 v7.4.6 Redis Open Source Install and upgrade Manage Redis Administration Security Configuration High availability with Sentinel Replication Scale with Redis Cluster Persistence Optimization Troubleshooting Debugging Redis Open Source and Redis Software Reference Redis Insight Redis Data Integration (RDI)
- Redis Cloud Redis for Kubernetes latest ▼ ▲ latest v7.22 v7.8.6 v7.8.4 v7.4.6 Redis Open Source Install and upgrade Manage Redis Administration Security Configuration High availability with Sentinel Replication Scale with Redis Cluster Persistence Optimization Troubleshooting Debugging Redis Open Source and Redis Software Reference Redis Insight Redis Data Integration (RDI)
- Redis for Kubernetes latest ▼ ▲ latest v7.22 v7.8.6 v7.8.4 v7.4.6 Redis Open Source Install and upgrade Manage Redis Administration Security Configuration High availability with Sentinel Replication Scale with Redis Cluster Persistence Optimization Troubleshooting Debugging Redis Open Source and Redis Software Reference Redis Insight Redis Data Integration (RDI)
- Redis Open Source Install and upgrade Manage Redis Administration Security Configuration High availability with Sentinel Replication Scale with Redis Cluster Persistence Optimization Troubleshooting Debugging Redis Open Source and Redis Software Reference Redis Insight Redis Data Integration (RDI)
- Install and upgrade Manage Redis Administration Security Configuration High availability with Sentinel Replication Scale with Redis Cluster Persistence Optimization Troubleshooting Debugging Redis Open Source and Redis Software Reference
- Manage Redis Administration Security Configuration High availability with Sentinel Replication Scale with Redis Cluster Persistence Optimization Troubleshooting Debugging Redis Open Source and Redis Software Reference
- Administration Security Configuration High availability with Sentinel Replication Scale with Redis Cluster Persistence Optimization Troubleshooting Debugging
- Security Configuration High availability with Sentinel Replication Scale with Redis Cluster Persistence Optimization Troubleshooting Debugging
- Configuration High availability with Sentinel Replication Scale with Redis Cluster Persistence Optimization Troubleshooting Debugging
- High availability with Sentinel Replication Scale with Redis Cluster Persistence Optimization Troubleshooting Debugging
- Replication Scale with Redis Cluster Persistence Optimization Troubleshooting Debugging
- Scale with Redis Cluster Persistence Optimization Troubleshooting Debugging
- Persistence Optimization Troubleshooting Debugging
- Optimization Troubleshooting Debugging
- Troubleshooting Debugging
- Debugging
- Redis Open Source and Redis Software Reference
- Reference
...
- â Redis Open Source
- â Manage Redis
- â High availability with Redis Sentinel
...
The Redis source distribution contains a file called sentinel.conf that is a self-documented example configuration file you can use to
...
At the end of the process it is possible to use the command SENTINEL MASTER mastername in order to check if all the Sentinels agree about
...
triggering a fail over Redis Sentinel will try to send a SCRIPT KILL command, that will only succeed if the script was read-only.
`SCRIPT KILL`
...
`requirepass`
In order for Sentinels to connect to Redis server instances when they are
configured with requirepass , the Sentinel configuration must include the sentinel auth-pass directive, in the format:
`requirepass`
`sentinel auth-pass`
`sentinel auth-pass <master-name> <password>`
...
local to a given Sentinel instance. Another is called Objectively Down condition (ODOWN) and is reached when enough Sentinels (at least the
...
### On this page
- Sentinel as a distributed system
- Sentinel quick start Obtaining Sentinel Running Sentinel Fundamental things to know about Sentinel before deploying Configuring Sentinel Other Sentinel options Example Sentinel deployments Sentinel, Docker, NAT, and possible issues IP Addresses and DNS names
- Obtaining Sentinel
- Running Sentinel
- Fundamental things to know about Sentinel before deploying
- Configuring Sentinel
...
- A quick tutorial
- Asking Sentinel about the state of a master
- Obtaining the address of the current master Testing the failover
- Testing the failover
- Sentinel API Sentinel commands Reconfiguring Sentinel at Runtime Adding or removing Sentinels Removing the old master or unreachable replicas Pub/Sub messages Handling of -BUSY state
- Sentinel commands
- Reconfiguring Sentinel at Runtime
- Adding or removing Sentinels
- Removing the old master or unreachable replicas
- Pub/Sub messages
- Handling of -BUSY state
- Replicas priority Sentinel and Redis authentication
- Sentinel and Redis authentication
- Redis Access Control List authentication Redis password-only authentication
- Redis password-only authentication
- Configuring Sentinel instances with authentication Sentinel Access Control List authentication Sentinel password-only authentication Sentinel clients implementation
- Sentinel Access Control List authentication
- Sentinel password-only authentication
- Sentinel clients implementation
- More advanced concepts SDOWN and ODOWN failure state
- SDOWN and ODOWN failure state
- Sentinels and replicas auto discovery
- Sentinel reconfiguration of instances outside the failover procedure Replica selection and priority
- Replica selection and priority
- Algorithms and internals Quorum Configuration epochs Configuration propagation Consistency under partitions
- Quorum
- Configuration epochs
- Configuration propagation
- Consistency under partitions
- Sentinel persistent state TILT mode
- TILT mode
#### Use Cases
#### Industries
#### Compare
