...
url: https://redis.io/docs/latest/operate/rc/databases/rdi/setup/
retrieved_utc: '2026-04-09T20:45:52.400425+00:00'
tags:
- official
- docs
- sitemap
fetched_url: https://redis.io/docs/latest/operate/rc/databases/rdi/setup/index.html.md
---
...
```json metadata
{
  ...
  "tableOfContents": {"sections":[{"id":"create-new-data-pipeline","title":"Create new data pipeline"},{"id":"prepare-source-database","title":"Prepare source database"},{"children":[{"id":"create-network-load-balancer-create-network-load-balancer-ec2","title":"Create network load balancer {#create-network-load-balancer-ec2}"},{"id":"create-endpoint-service-create-endpoint-service-ec2","title":"Create endpoint service {#create-endpoint-service-ec2}"},{"id":"create-network-load-balancer-create-network-load-balancer-rds","title":"Create network load balancer {#create-network-load-balancer-rds}"},{"id":"create-endpoint-service-create-endpoint-service-rds","title":"Create endpoint service {#create-endpoint-service-rds}"},{"id":"set-up-lambda-function-connectivity-setup-lambda-function","title":"Set up Lambda function connectivity {#setup-lambda-function}"}],"id":"set-up-connectivity","title":"Set up connectivity"},{"children":[{"id":"create-encryption-key","title":"Create encryption key"},{"id":"create-database-credentials-secrets","title":"Create database credentials secrets"}],"id":"share-source-database-credentials","title":"Share source database credentials"},{"id":"next-steps","title":"Next steps"}]}
...
Before using the pipeline, you must first prepare your source database to use the Debezium connector for change data capture (CDC). See [Prerequisites]() to find a list of supported source databases and database versions.

See [Prepare source databases]() to find steps for your database type:
...
See the [RDI architecture overview]() for more information about CDC.
...
If you have specific requirements that necessitate using RDS Proxy instead of the recommended Lambda function approach, see the [RDS Proxy setup guide](). Note that RDS Proxy is not recommended and does not work with PostgreSQL.
...
| `db_endpoint` | Your RDS cluster or instance endpoint | AWS Console â RDS â Your database â Connectivity |
| `db_port` | Your database port | AWS Console â RDS â Your database â Connectivity (default: `5432` for PostgreSQL, `3306` for MySQL, `1433` for SQL Server) |
| `elb_tg_arn` | The NLB Target Group ARN | AWS Console â EC2 â Target Groups â Your target group |
| `rds_arn` | The RDS cluster or instance ARN | AWS Console â RDS â Your database â Configuration |
| `rds_cluster_identifier` | The RDS cluster identifier | AWS Console â RDS â Your cluster name |
...
1. Check the Lambda function in AWS Console â Lambda â Functions
...
1. Check the SNS subscription in AWS Console â SNS â Subscriptions
...
After you have set up your source database and prepared connectivity and credentials, select **Define source database** to [define your source connection and data pipeline]().
