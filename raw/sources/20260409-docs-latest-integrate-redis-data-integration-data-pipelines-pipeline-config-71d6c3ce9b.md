...
url: https://redis.io/docs/latest/integrate/redis-data-integration/data-pipelines/pipeline-config/
retrieved_utc: '2026-04-09T20:45:52.727234+00:00'
tags:
- official
- docs
- sitemap
fetched_url: https://redis.io/docs/latest/integrate/redis-data-integration/data-pipelines/pipeline-config/index.html.md
---
...
```json metadata
{
  ...
  "group": "di",
  "tableOfContents": {"sections":[{"id":"example","title":"Example"},{"children":[{"id":"sources","title":"Sources"},{"id":"targets","title":"Targets"},{"id":"processors","title":"Processors"}],"id":"sections","title":"Sections"}]}
...
- `connection`: The connection details for the source database: `type`, `host`, `port`, 
  ...
  - If you use [TLS](https://en.wikipedia.org/wiki/Transport_Layer_Security)/
    ...
    secrets that you should set as described in [Set secrets]().
...
that you should set as described in [Set secrets]()
(it is not possible to include these certificates as plain text in the file).

If you specify `localhost` as the address of either the source or target server during
...
See also the
[RDI configuration file reference]()
