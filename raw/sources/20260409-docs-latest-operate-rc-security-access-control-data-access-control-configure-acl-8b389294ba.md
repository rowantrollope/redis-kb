...
url: https://redis.io/docs/latest/operate/rc/security/access-control/data-access-control/configure-acls/
retrieved_utc: '2026-04-09T20:45:55.197595+00:00'
tags:
- official
- docs
- sitemap
fetched_url: https://redis.io/docs/latest/operate/rc/security/access-control/data-access-control/configure-acls/index.html.md
---
...
```json metadata
{
  ...
  "tableOfContents": {"sections":[{"children":[{"id":"command-acl-rules","title":"Command ACL rules"},{"id":"command-category-acl-rules","title":"Command category ACL rules"},{"id":"key-acl-rules","title":"Key ACL rules"},{"id":"pubsub-acl-rules","title":"Pub/sub ACL rules"},{"id":"selectors","title":"Selectors"},{"id":"predefined-permissions","title":"Predefined permissions"},{"id":"advanced-capability-command-permissions","title":"Advanced capability command permissions"}],"id":"define-permissions-with-acl-syntax","title":"Define permissions with ACL syntax"}]}
...
You can define these permissions using the [Redis ACL syntax](#acl-rules). This syntax lets you concisely specify which commands, command categories, keys, and pub/sub channels to allow.
...
A [**command category**](#command-categories) is a predefined, named set of commands.
...
`read` command category, run the following command with [`redis-cli`]():
...
To specify which **keys** are accessible, use the [key permissions syntax](#key-permissions).
...
For more information on how this works, see the [key permissions syntax](#key-permissions).
...
Starting with Redis 7.0, Redis supports adding multiple sets of rules that are evaluated independently of each other, called [selectors](#selectors). 
