---
title: rladmin tune
url: https://redis.io/docs/latest/operate/rs/references/cli-utilities/rladmin/tune/
retrieved_utc: '2026-04-09T20:45:36.043175+00:00'
tags:
- official
- docs
- sitemap
fetched_url: https://redis.io/docs/latest/operate/rs/references/cli-utilities/rladmin/tune/index.html.md
---
...
```json metadata
{
  ...
  "tableOfContents": {"sections":[{"children":[{"id":"parameters","title":"Parameters"},{"id":"returns","title":"Returns"},{"id":"example","title":"Example"}],"id":"tune-cluster","title":"tune cluster"},{"children":[{"id":"parameters","title":"Parameters"},{"id":"returns","title":"Returns"},{"id":"example","title":"Example"}],"id":"tune-db","title":"tune db"},{"children":[{"id":"parameters","title":"Parameters"},{"id":"returns","title":"Returns"},{"id":"example","title":"Example"}],"id":"tune-node","title":"tune node"},{"children":[{"id":"parameters","title":"Parameters"},{"id":"returns","title":"Returns"},{"id":"example","title":"Example"}],"id":"tune-proxy","title":"tune proxy"}]}
...
| data_internode_encryption              | `enabled`<br />`disabled`       | Activates or deactivates [internode encryption]() for new databases    |
| db_conns_auditing                      | `enabled`<br /> `disabled`      | Activates or deactivates [connection auditing]() by default for new databases of a cluster                                                                  |
...
| default_non_sharded_proxy_policy | `single`<br /><br />`all-master-shards`<br /><br />`all-nodes` | Default [proxy policy]() for newly created non-sharded databases' endpoints |
...
| default_sharded_proxy_policy | `single`<br /><br />`all-master-shards`<br /><br />`all-nodes` | Default [proxy policy]() for newly created sharded databases' endpoints |
| default_shards_placement | `dense`<br />`sparse` | New databases place shards according to the default [shard placement policy]() |
...
| failure_detection_sensitivity | `high`<br />`low` | Predefined thresholds and timeouts for failure detection (previously known as `watchdog_profile`)<br />â¢ `high` (previously `local-network`) â high failure detection sensitivity, lower thresholds, faster failure detection and failover<br />â¢ `low` (previously `cloud`) â low failure detection sensitivity, higher tolerance for latency variance (also called network jitter) |
...
| slave_ha                               | `enabled` <br /> `disabled`   | Activates or deactivates [replica high availability]() in the cluster<br />(enabled by default; use [`rladmin tune db`](#tune-db) to change `slave_ha` for a specific database)<br /><br />Deprecated as of Redis Software v7.2.4. |
...
| watchdog_profile                       | `cloud` <br /> `local-network` | Watchdog profiles with preconfigured thresholds and timeouts (deprecated as of Redis Software v6.4.2-69; use <span class="break-all">`failure_detection_sensitivity`</span> instead)<br />â¢ `cloud` is suitable for common cloud environments and has a higher tolerance for latency variance (also called network jitter).<br />â¢ `local-network` is suitable for dedicated LANs and has better failure detection and failover times. |
...
| data_internode_encryption            | `enabled`<br /> `disabled`       | Activates or deactivates [internode encryption]() for the database               |
| db_conns_auditing                    | `enabled`<br /> `disabled`       | Activates or deactivates database [connection auditing]() for a database                                                                 |
...
| quorum_only          | `enabled`<br /> `disabled` | If activated, configures the node as a [quorum-only node]()                         |
