...
url: https://redis.io/docs/latest/operate/kubernetes/active-active/create-aa-crdb-cli/
retrieved_utc: '2026-04-09T20:46:01.054033+00:00'
tags:
- official
- docs
- sitemap
fetched_url: https://redis.io/docs/latest/operate/kubernetes/active-active/create-aa-crdb-cli/index.html.md
---
...
```json metadata
{
  ...
  "tableOfContents": {"sections":[{"id":"prerequisites","title":"Prerequisites"},{"id":"document-required-parameters","title":"Document required parameters"},{"children":[{"id":"using-ingress-controller","title":"Using ingress controller"},{"id":"using-openshift-routes","title":"Using OpenShift routes"}],"id":"add-activeactive-section-to-the-rec-resource-file","title":"Add activeActive section to the REC resource file"},{"id":"create-an-active-active-database-with-crdb-cli","title":"Create an Active-Active database with crdb-cli"},{"id":"test-your-database","title":"Test your database"}]}

,
  "codeExamples": []
}
``` Versions 6.4.2 and later support the Active-Active database controller. This controller allows you to create Redis Enterprise Active-Active databases (REAADB) and Redis Enterprise remote clusters (RERC) with custom resources. We recommend using the [REAADB method for creating Active-Active databases]().
...
You'll need to create DNS aliases to resolve your API hostname `<api-hostname>`,`<ingress-suffix>`, `<replication-hostname>` to the IP address for the ingress controllerâs LoadBalancer (or routes in Openshift) for each database. To avoid entering multiple DNS records, you can use a wildcard in your alias (such as *.ijk.example.com).
...
4. Make sure you have DNS aliases for each database that resolve your API hostname `<api-hostname>`,`<ingress-suffix>`, `<replication-hostname>` to the IP address of the ingress controllerâs LoadBalancer. To avoid entering multiple DNS records, you can use a wildcard in your alias (such as `*.ijk.example.com`).
