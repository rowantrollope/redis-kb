---
title: BDB object
url: https://redis.io/docs/latest/operate/rs/references/rest-api/objects/bdb/
retrieved_utc: '2026-04-09T20:46:02.037631+00:00'
tags:
- official
- docs
- sitemap
fetched_url: https://redis.io/docs/latest/operate/rs/references/rest-api/objects/bdb/index.html.md
---
...
| background_op | Deprecated as of Redis Software v7.8.2. Use [`GET /v1/actions/bdb/<bdb_uid>`]() instead.<br />[{<br />  "status": string,<br />  "name": string,<br />  "error": object,<br />  "progress": number<br />}, ...] (read-only); **progress**: Percent of completed steps in current operation |
...
| bigstore_version | The database's `bigstore_version`:<br />â¢ `1` for Auto Tiering (Redis on Flash version 1). Default version.<br/>â¢ `2` for Redis Flex (Redis on Flash version 2) on databases that support it. Can only be used with the `speedb` driver.<br /><br />You can only choose the `bigstore_version` for databases with Redis version 7.4. Database versions earlier than 7.4 use `bigstore_version` `1`. Database versions 8.0 and later use `bigstore_version` `2`. |
...
| conns_type | Connections limit type<br />Values:<br />**âper-threadâ**<br />âper-shardâ |
...
| crdt_xadd_id_uniqueness_mode | XADD strict ID uniqueness mode. CRDT only.<br />Values:<br />âliberalâ<br />**âstrictâ**<br />âsemi-strictâ |
...
| max_pipelined | integer (default:&nbsp;2000); Determines the maximum number of commands in the proxyâs pipeline per shard connection. |
...
| multi_commands_opt | If set to `batch`, it reduces the overhead of transaction management by batching multiple commands into a single transaction.<br />Values:<br />**disabled**: Turns off the optimization.<br />**batch**: Enables the optimization.<br />**auto**: Uses the [cluster's `multi_commands_opt` setting](). |
...
| <span class="break-all">oss_cluster_api_preferred_endpoint_type</span> | Endpoint type in the OSS cluster API<br />Values:<br />**âipâ**<br />âhostnameâ |
...
| probabilistic | [complex object](); Configuration fields for probabilistic data structures. |
...
| query_performance_factor | [complex object](); Configures query performance factor and related fields |
...
| search | [complex object](); Configuration fields for search and query. |
...
| timeseries | [complex object](); Configuration fields for timeseries. |
