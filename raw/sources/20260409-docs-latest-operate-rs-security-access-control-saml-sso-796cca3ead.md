...
url: https://redis.io/docs/latest/operate/rs/security/access-control/saml-sso/
retrieved_utc: '2026-04-09T20:45:58.698167+00:00'
tags:
- official
- docs
- sitemap
fetched_url: https://redis.io/docs/latest/operate/rs/security/access-control/saml-sso/index.html.md
---
...
```json metadata
{
  ...
  "tableOfContents": {"sections":[{"children":[{"id":"idp-initiated-sso","title":"IdP-initiated SSO"},{"id":"sp-initiated-sso","title":"SP-initiated SSO"}],"id":"sso-overview","title":"SSO overview"},{"id":"idp-requirements","title":"IdP requirements"},{"children":[{"id":"upload-sp-certificate","title":"Upload SP certificate"},{"id":"download-sp-metadata","title":"Download SP metadata"},{"id":"set-up-saml-app-set-up-app","title":"Set up SAML app {#set-up-app}"},{"id":"download-idp-metadata","title":"Download IdP metadata"},{"id":"configure-idp-metadata-in-redis-software-configure-idp-metadata","title":"Configure IdP metadata in Redis Software {#configure-idp-metadata}"},{"id":"assign-saml-app-to-existing-users","title":"Assign SAML app to existing users"},{"id":"activate-sso-activate-sso","title":"Activate SSO {#activate-sso}"}],"id":"set-up-saml-sso","title":"Set up SAML SSO"},{"id":"add-new-users-with-jit-provisioning","title":"Add new users with JIT provisioning"},{"id":"enforce-sso","title":"Enforce SSO"},{"children":[{"id":"change-sp-address","title":"Change SP address"}],"id":"update-configuration-update-config","title":"Update configuration {#update-config}"},{"id":"deactivate-sso","title":"Deactivate SSO"}]}
...
1. Create a service provider certificate for Redis Software. See [Create certificates ]() for instructions.
...
To upload a certificate using the REST API, use an [update cluster certificates]() request.
...
To download the service provider's metadata using the REST API, use a [get SAML service provider metadata]() request.
...
1. Upload your SAML app's assertion signing certificate using an [update cluster certificates]() REST API request.
...
1. Configure the identity provider metadata using an [update SSO configuration]() REST API request.
...
To activate single sign-on using the REST API, use an [update SSO configuration]() request.
...
To see a list of available role UIDs in your cluster, use a REST API request to [get all roles]():
...
To enforce single sign-on using the REST API, use an [update SSO configuration]() request.
...
To change the service provider address, use an [update SSO configuration]() REST API request:
...
To deactivate single sign-on using the REST API, use an [update SSO configuration]() request.
