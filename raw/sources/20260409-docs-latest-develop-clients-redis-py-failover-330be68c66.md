...
url: https://redis.io/docs/latest/develop/clients/redis-py/failover/
retrieved_utc: '2026-04-09T20:45:58.910294+00:00'
tags:
- official
- docs
- sitemap
fetched_url: https://redis.io/docs/latest/develop/clients/redis-py/failover/index.html.md
---
...
```json metadata
{
  ...
  "topics": ["failover","failback","resilience","health checks","retries"],
  "relatedPages": ["/develop/clients/failover"],
  "scope": ["client-specific","implementation"],
  "tableOfContents": {"sections":[{"children":[{"id":"endpoint-configuration","title":"Endpoint configuration"},{"id":"client-configuration","title":"Client configuration"},{"id":"circuit-breaker-configuration","title":"Circuit breaker configuration"},{"id":"retry-configuration","title":"Retry configuration"},{"id":"failback-configuration","title":"Failback configuration"},{"id":"general-failover-configuration","title":"General failover configuration"},{"id":"failover-callbacks","title":"Failover callbacks"}],"id":"failover-configuration","title":"Failover configuration"},{"children":[{"id":"pinghealthcheck-default","title":"PingHealthCheck (default)"},{"id":"lagawarehealthcheck-redis-software-only-lag-aware-health-check","title":"LagAwareHealthCheck (Redis Software only) {#lag-aware-health-check}"},{"id":"custom-health-check-strategy","title":"Custom health check strategy"}],"id":"health-check-configuration","title":"Health check configuration"},{"children":[{"id":"manual-failback","title":"Manual failback"}],"id":"managing-databases-at-runtime","title":"Managing databases at runtime"},{"id":"behavior-when-all-endpoints-are-unhealthy","title":"Behavior when all endpoints are unhealthy"},{"children":[{"id":"excessive-or-constant-health-check-failures","title":"Excessive or constant health check failures"},{"id":"slow-failback-after-recovery","title":"Slow failback after recovery"}],"id":"troubleshooting","title":"Troubleshooting"}]}
...
(see [Selecting a failover target]() for a full description of how
...
| Option | Description |
| --- | --- |
| `client_kwargs` | Keyword parameters to pass to the internal client constructor for this endpoint. Use it to specify the host, port, username, password, and other connection parameters (see [Connect to the server]() for more information). This is especially useful if you are using a custom client class (see [Client configuration](#client-configuration) below for more information). |
| `from_url` | Redis URL to connect to this endpoint, as an alternative to passing the host and port in `client_kwargs`. |
| `from_pool` | A `ConnectionPool` to supply the endpoint connection (see [Connect with a connection pool]() for more information) |
...
(see [Detecting connection problems]() for more information on how the
...
test (such as a [`PING`]() command) to determine if the
...
in [Retries]()). However, if the client exhausts
