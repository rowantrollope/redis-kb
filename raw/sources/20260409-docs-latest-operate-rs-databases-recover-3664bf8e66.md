...
url: https://redis.io/docs/latest/operate/rs/databases/recover/
retrieved_utc: '2026-04-09T20:45:56.869845+00:00'
tags:
- official
- docs
- sitemap
fetched_url: https://redis.io/docs/latest/operate/rs/databases/recover/index.html.md
---
...
```json metadata
{
  ...
  "tableOfContents": {"sections":[{"id":"prerequisites","title":"Prerequisites"},{"id":"recover-databases","title":"Recover databases"},{"id":"configure-automatic-recovery","title":"Configure automatic recovery"}]}
...
- Before you start database recovery, make sure that the cluster that hosts the database is healthy.
    ...
    you must [recover the cluster]() before you recover the databases.
...
you can run the recovery process from the [`rladmin`]()
...
1. To see which databases are recoverable, run [`rladminÂ recover list`]():
...
1. Recover the database using one of the following [`rladmin recover`]() commands:
...
To enable automatic recovery using `rladmin`, run the [`rladmin tune cluster`]() command:
...
To enable automatic recovery using the REST API, use an [update cluster policy]() request:
