...
retrieved_utc: '2026-04-09T20:45:56.559637+00:00'
tags:
- official
- docs
- sitemap
fetched_url: https://redis.io/docs/latest/develop/using-commands/transactions/index.html.md
---
...
**even when a command fails, all the other commands in the queue are processed** â Redis will _not_ stop the
...
so collisions are unlikely â usually there's no need to repeat the operation.

Starting with version 8.4, Redis offers new atomic compare-and-set and compare-and-delete commands for string keys.
A client can use a single ([`SET`]() command with the `IFEQ`/`IFNE`/`IFDEQ`/`IFDNE` options to atomically update a string key only if its value has not changed since it was fetched.
This is much simpler and faster.
Similarly, a client can use a single [`DELEX`]() command for compare-and-delete: atomically deleting a string key if its value hasn't changed since it was fetched.
