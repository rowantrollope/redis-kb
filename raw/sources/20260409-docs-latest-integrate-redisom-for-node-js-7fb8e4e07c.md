...
url: https://redis.io/docs/latest/integrate/redisom-for-node-js/
retrieved_utc: '2026-04-09T20:45:53.886115+00:00'
tags:
- official
- docs
- sitemap
fetched_url: https://redis.io/docs/latest/integrate/redisom-for-node-js/index.html.md
---
...
```json metadata
{
  ...
  "group": "library",
  "tableOfContents": {"sections":[{"id":"what-is-redis-om","title":"What is Redis OM?"},{"id":"prerequisites","title":"Prerequisites"},{"id":"starter-code","title":"Starter code"},{"id":"configure-and-run","title":"Configure and run"},{"id":"setting-up-a-client","title":"Setting up a Client"},{"children":[{"id":"entity","title":"Entity"},{"id":"schema","title":"Schema"},{"id":"repository","title":"Repository"}],"id":"entity-schema-and-repository","title":"Entity, Schema, and Repository"},{"children":[{"id":"creating-a-person","title":"Creating a Person"},{"id":"reading-a-person","title":"Reading a Person"},{"id":"updating-a-person","title":"Updating a Person"},{"id":"deleting-a-person","title":"Deleting a Person"},{"id":"all-the-crud","title":"All the CRUD"}],"id":"set-up-the-person-router","title":"Set up the Person Router"},{"children":[{"id":"search-all-the-things","title":"Search all the things"},{"id":"full-text-search","title":"Full-text search"},{"id":"searching-the-globe","title":"Searching the globe"}],"id":"preparing-to-search","title":"Preparing to search"},{"id":"adding-location-tracking","title":"Adding location tracking"},{"id":"using-node-redis","title":"Using Node Redis"},{"id":"storing-location-history-with-streams","title":"Storing location history with Streams"},{"id":"wrap-up","title":"Wrap-up"}]}
...
- [Redis Stack](): You need a version of Redis Stack, either running locally on your machine or [in the cloud](https://redis.com/try-free/?utm_source=redisio&utm_medium=referral&utm_campaign=2023-09-try_free&utm_content=cu-redis_cloud_users).
...
The `persons` folder has some JSON files and a shell script. The JSON files are sample personsâall musicians because funâthat you can load into the API to test it. The shell scriptâ`load-data.sh`âwill load all the JSON files into the API using `curl`.
...
Next, we need to define an **entity**. An `Entity` is the class that holds you data when you work with itâthe thing being mapped to. It is what you create, read, update, and delete. Any class that extends `Entity` is an entity. We'll define our `Person` entity with a single line:
...
The first three do exactly what you thinkâthey define a property that is a [`String`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/String), a [`Number`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Number), or a [`Boolean`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Boolean). `string[]` does what you'd think as well, specifically defining an [`Array`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Array) of strings.
...
A `text` field is a lot like a `string`. If you're just reading and writing objects, they are identical. But if you want to *search* on them, they are very, very different. We'll talk about search more later, but the tl;dr is that `string` fields can only be matched on their whole valueâno partial matchesâand are best for keys while `text` fields have full-text search enabled on them and are optimized for human-readable text.
...
This is exactly what we handed it with one exception: the `entityId`. Every entity in Redis OM has an entity ID which isâas you've probably guessedâthe unique ID of that entity. It was randomly generated when we called `.createAndSave()`. Yours will be different, so make note of it.
...
This code extracts a parameter from the URL used in the routeâthe `entityId` that we received previously. It uses the `.fetch()` method on the `personRepository` to retrieve a `Person` using that `entityId`. Then, it returns that `Person`.
...
Deletionâmy favorite! Remember kids, deletion is 100% compression. The route that deletes is just as straightforward as the one that reads, but much more destructive:
...
{"entityId":"01FY9Z4RV2QHH0Z1GJM5ND15JE","firstName":"Rupert","lastName":"Holmes","age":75,"verified":true,"location":{"longitude":-2.518,"latitude":53.259},"locationUpdated":"2022-01-01T12:00:00.000Z","skills":["singing","songwriting","playwriting"],"personalStatement":"I like piÃ±a coladas and taking walks in the rain."} <- rupert-holmes.json
...
Note the use of the `.matches()` function. This is the only one that works with `text` fields. It takes a string that can be one or more wordsâspace-delimitedâthat you want to query for. Let's try it out. In Swagger, use this route to search for the word "walk". You should get the following results:
...
[
  {
    ...
    "personalStatement": "I like piÃ±a coladas and taking walks in the rain."
  },
  {
...
Here we're calling `.fetch()` to fetch a person, we're updating some values for that personâthe `.location` property with our longitude and latitude and the `.locationUpdated` property with the current date and time. Easy stuff.
...
`.xAdd()` takes a key name, an event ID, and a JavaScript object containing the keys and values that make up the event, i.e. the event data. For the key name, we're building a string using the `.keyName` property that `Person` inherited from `Entity` (which will return something like `Person:01FYC7CTPKYNXQ98JSTBC37AS1`) combined with a hard-coded value. We're passing in `*` for our event ID, which tells Redis to just generate it based on the current time and previous event ID. And we're passing in the locationâwith properties of longitude and latitudeâas our event data.
...
This tells Redis to get a range of values from a Stream stored in the given the key nameâ`Person:01FYC7CTPKYNXQ98JSTBC37AS1:locationHistory` in our example. The next values are the starting event ID and the ending event ID. `-` is the beginning of the Stream. `+` is the end. So this returns everything in the Stream:
