...
url: https://redis.io/docs/latest/operate/rs/references/rest-api/requests/bdbs/upgrade/
retrieved_utc: '2026-04-09T20:45:36.232109+00:00'
tags:
- official
- docs
- sitemap
fetched_url: https://redis.io/docs/latest/operate/rs/references/rest-api/requests/bdbs/upgrade/index.html.md
---
...
```json metadata
{
  ...
  "tableOfContents": {"sections":[{"children":[{"id":"request-post-request","title":"Request {#post-request}"},{"id":"response-post-response","title":"Response {#post-response}"},{"id":"status-codes-post-status-codes","title":"Status codes {#post-status-codes}"}],"id":"upgrade-database-post-bdbs-upgrade","title":"Upgrade database {#post-bdbs-upgrade}"}]}
...
| modules | list of modules | List of dicts representing the modules that will be upgraded. As of Redis Software v7.8.2, `current_module` and `new_module` are deprecated.<br></br>Each dict includes:<br></br>â¢ `current_module`: uid of a module to upgrade (deprecated)<br></br>â¢ `new_module`: uid of the module we want to upgrade to (deprecated)<br></br>â¢ `new_module_args`: args list for the new module (no defaults for the three module-related parameters).
