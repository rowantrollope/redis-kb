---
title: "Redis replication official docs"
url: "https://redis.io/docs/latest/operate/oss_and_stack/management/replication/"
retrieved_utc: "2026-04-09T20:34:18.403311+00:00"
tags: ['ad-hoc']
---
...
- When a master and a replica instance are well-connected, the master keeps the replica updated by sending a stream of commands to the replica to replicate the effects on the dataset happening on the master side due to: client writes, keys expired or evicted, any other action changing the master dataset.
...
in memory by the master to perform the partial resynchronization. See the example redis.conf shipped with the Redis distribution for more information.
`redis.conf`
...
arrive after the first one is controlled by the repl-diskless-sync-delay parameter. Please refer to the example redis.conf file in the Redis distribution
for more details.
`repl-diskless-sync`
