...
url: https://redis.io/docs/latest/operate/rs/networking/cluster-lba-setup/
retrieved_utc: '2026-04-09T20:46:02.980388+00:00'
tags:
- official
- docs
- sitemap
fetched_url: https://redis.io/docs/latest/operate/rs/networking/cluster-lba-setup/index.html.md
---
...
```json metadata
{
  ...
  "tableOfContents": {"sections":[{"id":"dns-role-for-databases","title":"DNS role for databases"},{"id":"network-architecture-with-load-balancer","title":"Network architecture with load balancer"},{"children":[{"id":"prerequisites","title":"Prerequisites"},{"id":"configure-load-balancers","title":"Configure load balancers"},{"id":"configure-cluster","title":"Configure cluster"},{"id":"configure-database","title":"Configure database"},{"id":"update-load-balancer-configuration-when-cluster-configuration-changes","title":"Update load balancer configuration when cluster configuration changes"}],"id":"set-up-a-cluster-with-load-balancers","title":"Set up a cluster with load balancers"},{"children":[{"id":"replica-of","title":"Replica Of"},{"id":"active-active","title":"Active-Active"}],"id":"intercluster-communication-considerations","title":"Intercluster communication considerations"}]}
...
- [Install]() the latest version of Redis Software on your clusters
...
Other ports are shown in the list of [Redis Software network ports]().
...
For clusters behind load balancers, we recommend using the `all-nodes` [proxy policy]() and enabling `handle_redirects`.
...
After you update the cluster settings and configure the load balancers, you can go to the Redis Software Cluster Manager UI at `https://load-balancer-virtual-ip:8443/` and [create a new database]().

To create an Active-Active database, use the `crdb-cli` utility. See the [`crdb-cli` reference]() for more information about creating Active-Active databases from the command line.
...
Redis Software supports several topologies that allow intercluster replication, such as [Replica Of]() and [Active-Active]() deployment options.
