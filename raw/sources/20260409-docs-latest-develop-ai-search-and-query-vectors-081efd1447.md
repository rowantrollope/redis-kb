...
retrieved_utc: '2026-04-09T20:45:53.801535+00:00'
tags:
- official
- docs
- sitemap
fetched_url: https://redis.io/docs/latest/develop/ai/search-and-query/vectors/index.html.md
---
...
```json metadata
{
  ...
  "tableOfContents": {"sections":[{"id":"overview","title":"Overview"},{"children":[{"id":"flat-index","title":"FLAT index"},{"id":"hnsw-index","title":"HNSW index"},{"id":"svs-vamana-index","title":"SVS-VAMANA index"},{"id":"distance-metrics","title":"Distance metrics"}],"id":"create-a-vector-index","title":"Create a vector index"},{"children":[{"id":"hash","title":"Hash"},{"id":"json","title":"JSON"}],"id":"store-and-update-vectors","title":"Store and update vectors"},{"children":[{"id":"knn-vector-search","title":"KNN vector search"},{"id":"vector-range-queries","title":"Vector range queries"},{"id":"filters","title":"Filters"},{"id":"how-filtering-works","title":"How filtering works"}],"id":"search-with-vectors","title":"Search with vectors"},{"children":[{"id":"filter-mode","title":"Filter mode"},{"id":"cluster-specific-query-parameters","title":"Cluster-specific query parameters"},{"id":"index-specific-query-parameters","title":"Index-specific query parameters"},{"id":"important-notes","title":"Important notes"}],"id":"runtime-query-parameters","title":"Runtime query parameters"},{"children":[{"id":"knn-vector-search-examples","title":"KNN vector search examples"},{"id":"filter-examples","title":"Filter examples"},{"id":"cluster-optimization-examples","title":"Cluster optimization examples"},{"id":"cluster-considerations-and-best-practices","title":"Cluster considerations and best practices"},{"id":"range-query-examples","title":"Range query examples"}],"id":"vector-search-examples","title":"Vector search examples"},{"id":"memory-consumption-comparison","title":"Memory consumption comparison"},{"id":"next-steps","title":"Next steps"},{"id":"continue-learning-with-redis-university","title":"Continue learning with Redis University"}]}
...
1. [**Configure vector queries at runtime**]().
1. [**Vector search examples**](): Explore several vector search examples that cover different use cases and techniques.
...
| Attribute                  | Description                              | Default value |
|:---------------------------|:-----------------------------------------|:-------------:|
| `COMPRESSION`              | Compression algorithm; one of `LVQ8`, `LVQ4`, `LVQ4x4`, `LVQ4x8`, `LeanVec4x8`, or `LeanVec8x8`. See [this page]() for more information. | none |
...
| `GRAPH_MAX_DEGREE`         | Sets the maximum number of edges per node; equivalent to `HNSWâs M*2`. A higher max degree may yield a higher quality graph in terms of recall for performance, but the memory footprint of the graph is directly proportional to the maximum degree. | 32 |
...
|:-----------------|:------------|:------------|:--------|
| `$SHARD_K_RATIO` | Ratio of `top_k` results to fetch per shard in cluster setups. Calculation: `shard_results = top_k Ã ratio`. | 0.1 - 1.0 (up to 2 decimal places) | 1.0 |

**Important considerations:**

- Only applicable to vector KNN queries in Redis cluster environments
- Each shard must retrieve at least `max(top_k / #shards, ceil(top_k Ã ratio))` results
...
Below are a number of examples to help you get started. For more comprehensive walkthroughs, see the [Redis vector quickstart guide]() and the [Redis AI Resources](https://github.com/redis-developer/redis-ai-resources) Github repo.
...
Each shard must retrieve at least `max(top_k / #shards, ceil(top_k Ã ratio))` results to ensure the cluster can return the requested `top_k` results. If your specified ratio would result in fewer results per shard than this minimum, Redis automatically adjusts to the minimum required ratio.

**Example scenarios:**
- `top_k=100`, 4 shards, `ratio=0.5` â 50 results per shard (valid)
- `top_k=100`, 4 shards, `ratio=0.2` â 25 results per shard (minimum: 25, so valid)
- `top_k=100`, 8 shards, `ratio=0.1` â 10 results per shard (minimum: 13, so Redis uses 13)
