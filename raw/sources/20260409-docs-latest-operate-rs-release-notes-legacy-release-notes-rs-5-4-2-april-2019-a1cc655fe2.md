...
url: https://redis.io/docs/latest/operate/rs/release-notes/legacy-release-notes/rs-5-4-2-april-2019/
retrieved_utc: '2026-04-09T20:45:59.936876+00:00'
tags:
- official
- docs
- sitemap
fetched_url: https://redis.io/docs/latest/operate/rs/release-notes/legacy-release-notes/rs-5-4-2-april-2019/index.html.md
---
...
```json metadata
{
  ...
  "tableOfContents": {"sections":[{"id":"overview","title":"Overview"},{"children":[{"id":"active-active-redis-crdb-activeactive-redis-crdb","title":"Active-Active Redis (CRDB) {#activeactive-redis-crdb}"},{"id":"new-backup-destinations-sftp-and-mount-point","title":"New backup destinations - SFTP and mount point"},{"id":"email-alerts-per-database-per-team-member","title":"Email alerts per database (per team member)"},{"id":"optional-client-authentication-tls","title":"Optional client authentication (TLS)"},{"id":"node-maintenance-mode","title":"Node maintenance mode"}],"id":"new-features","title":"New features"},{"id":"additional-capabilities","title":"Additional capabilities"},{"id":"information","title":"Information"},{"id":"important-fixes","title":"Important fixes"},{"children":[{"id":"upgrade","title":"Upgrade"},{"id":"cluster-api","title":"Cluster API"},{"id":"redis-commands","title":"Redis commands"}],"id":"known-limitations","title":"Known limitations"}]}
...
To take advantage of these updates with existing CRDBs you must [upgrade the CRDB protocol](#upgrading-crdbs/).

### New backup destinations - SFTP and mount point

The storage options for [periodic database backup](), [data export](https://docs.redis.com/latest/rs/databases/import-export/export-data.md), and [data import](https://docs.redis.com/latest/rs/databases/import-export/import-data/) are extended to include:
...
Improved distribution of [email alerts]() for cluster or database alerts.
...
You can now fine tune the [TLS configuration](), and ease certificates management by excluding client authentication enforcement,
...
Starting with RS 5.4.2 you can use [maintenance mode]() to handle this process simply and efficiently.

## Additional capabilities

- Support for open source Redis version 5.0.4
- Viewer roles were further limited to improve security compliance. The DB Viewer and Cluster Viewer roles cannot view the Redis password of a database. The DB Viewer role also cannot view the âlogâ screen.
...
- RS23508 - Fixed a failure in ârladmin statusâ command
...
- Before you upgrade a database with RediSearch Module to Redis 5.0,
    the RediSearch [Module must be upgraded]() to version 1.4.2 or above.
- Node upgrade fails if the SSL certificates were configured in version 5.0.2 and above by updating the certificates on the disk instead of using the new API.
    For assistance with this issue, see [SSL/TLS certificate updates]() in our documentation or [contact Redis support](https://redislabs.com/company/support/).
