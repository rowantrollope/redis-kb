...
url: https://redis.io/docs/latest/operate/rs/references/rest-api/objects/cluster_settings/
retrieved_utc: '2026-04-09T20:46:03.155388+00:00'
tags:
- official
- docs
- sitemap
fetched_url: https://redis.io/docs/latest/operate/rs/references/rest-api/objects/cluster_settings/index.html.md
---
...
| default_bigstore_version | **`1`**<br />`2` | Determines the default value of `bigstore_version` for new databases with Redis version 7.4:<br />â¢ `1` for Auto Tiering (Redis on Flash version 1). Default version.<br/>â¢ `2` for Redis Flex (Redis on Flash version 2) on databases that support it. Can only be used with the `speedb` driver. |
...
| <span class="break-all">failure_detection_sensitivity</span> | `high`<br />`low` | Predefined thresholds and timeouts for failure detection (previously known as <span class="break-all">`watchdog_profile`</span>)<br />â¢ `high` (previously `local-network`) â high failure detection sensitivity, lower thresholds, faster failure detection and failover<br />â¢ `low` (previously `cloud`) â low failure detection sensitivity, higher tolerance for latency variance (also called network jitter) |
