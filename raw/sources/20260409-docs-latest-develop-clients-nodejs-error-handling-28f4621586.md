...
retrieved_utc: '2026-04-09T20:45:53.385183+00:00'
tags:
- official
- docs
- sitemap
fetched_url: https://redis.io/docs/latest/develop/clients/nodejs/error-handling/index.html.md
---
...
```json metadata
{
  ...
  "relatedPages": ["/develop/clients/error-handling","/develop/clients/nodejs/produsage"],
  "scope": "implementation",
  "tableOfContents": {"sections":[{"id":"common-error-types","title":"Common error types"},{"id":"asyncawait-in-examples","title":"Async/await in examples"},{"id":"error-events","title":"Error events"},{"children":[{"id":"pattern-1-fail-fast","title":"Pattern 1: Fail fast"},{"id":"pattern-2-graceful-degradation","title":"Pattern 2: Graceful degradation"},{"id":"pattern-3-retry-with-backoff","title":"Pattern 3: Retry with backoff"},{"id":"pattern-4-log-and-continue","title":"Pattern 4: Log and continue"}],"id":"applying-error-handling-patterns","title":"Applying error handling patterns"},{"id":"see-also","title":"See also"}]}
...
|---|---|---|---|
| `ECONNREFUSED` | Connection refused | â | Retry with backoff or fall back |
| `ETIMEDOUT` | Command timeout | â | Retry with backoff |
| `ECONNRESET` | Connection reset by peer | â | Retry with backoff |
| `EAI_AGAIN` | DNS resolution failure | â | Retry with backoff |
| `ReplyError` (`WRONGTYPE`) | Type mismatch | â | Fix schema or code |
| `ReplyError` (`BUSY`, `TRYAGAIN`, `LOADING`) | Redis busy/loading | â ï¸ | Retry with backoff (bounded) |
