...
retrieved_utc: '2026-04-09T20:46:05.429932+00:00'
tags:
- official
- docs
- sitemap
fetched_url: https://redis.io/docs/latest/develop/ai/redisvl/api/cache/index.html.md
---
...
```json metadata
{
  ...
  "description": "",
  "categories": null,
  "tableOfContents": {"sections":[{"children":[{"id":"class-semanticcachenamellmcache-distancethreshold01-ttlnone-vectorizernone-filterablefieldsnone-redisclientnone-redisurlredislocalhost6379-connectionkwargs-overwritefalse-kwargs","title":"class SemanticCache(name='llmcache', distancethreshold=0.1, ttl=None, vectorizer=None, filterablefields=None, redisclient=None, redisurl='redis://localhost:6379', connection_kwargs={}, overwrite=False, **kwargs)"}],"id":"semanticcache","title":"SemanticCache"},{"children":[{"id":"class-embeddingscachenameembedcache-ttlnone-redisclientnone-asyncredisclientnone-redisurlredislocalhost6379-connectionkwargs","title":"class EmbeddingsCache(name='embedcache', ttl=None, redisclient=None, asyncredisclient=None, redisurl='redis://localhost:6379', connection_kwargs={})"}],"id":"embeddingscache","title":"EmbeddingsCache"}]}

,
  "codeExamples": [{"id":"semantic-cache-api"},{"id":"embeddings-cache-api"}]
}
```
...
* **Parameters:**
  ...
  * **vectorizer** (*Optional* *[* *BaseVectorizer* *]* *,* *optional*) â The vectorizer for the cache.
    Defaults to HFTextVectorizer.
  * **filterable_fields** (*Optional* *[* *List* *[* *Dict* *[* *str* *,* *Any* *]* *]* *]*) â An optional list of RedisVL fields
    that can be used to customize cache retrieval with filters.
  * **redis_client** (*Optional* *[* *Redis* *]* *,* *optional*) â A redis client connection instance.
    Defaults to None.
  * **redis_url** (*str* *,* *optional*) â The redis url. Defaults to redis://localhost:6379.
  * **connection_kwargs** (*Dict* *[* *str* *,* *Any* *]*) â The connection arguments
    for the redis client. Defaults to empty {}.
  * **overwrite** (*bool*) â Whether or not to force overwrite the schema for
    the semantic cache index. Defaults to false.
* **Raises:**
  * **TypeError** â If an invalid vectorizer is provided.
  * **TypeError** â If the TTL value is not an int.
  * **ValueError** â If the threshold is not between 0 and 1.
  * **ValueError** â If existing schema does not match new schema and overwrite is False.
...
* **Parameters:**
  * **prompt** (*Optional* *[* *str* *]* *,* *optional*) â The text prompt to search for in
    the cache.
  * **vector** (*Optional* *[* *List* *[* *float* *]* *]* *,* *optional*) â The vector representation
    of the prompt to search for in the cache.
  * **num_results** (*int* *,* *optional*) â The number of cached results to return.
    Defaults to 1.
  * **return_fields** (*Optional* *[* *List* *[* *str* *]* *]* *,* *optional*) â The fields to include
    in each returned result. If None, defaults to all available
    fields in the cached entry.
  * **filter_expression** (*Optional* *[*[*FilterExpression*]() *]*) â Optional filter expression
    that can be used to filter cache results. Defaults to None and
    the full cache will be searched.
  * **distance_threshold** (*Optional* *[* *float* *]*) â The threshold for semantic
    vector distance.
* **Returns:**
...
* **Return type:**
  List[Dict[str, Any]]
* **Raises:**
  * **ValueError** â If neither a prompt nor a vector is specified.
  * **ValueError** â if âvectorâ has incorrect dimensions.
  * **TypeError** â If return_fields is not a list when provided.
...
* **Parameters:**
  * **ids** (*Optional* *[* *List* *[* *str* *]* *]*) â List of entry IDs to remove from the cache.
    Entry IDs are the unique identifiers without the cache prefix.
  * **keys** (*Optional* *[* *List* *[* *str* *]* *]*) â List of full Redis keys to remove from the cache.
...
* **Raises:**
  **ValueError** â If neither ids nor keys is provided.
* **Parameters:**
  * **ids** (*List* *[* *str* *]*  *|* *None*)
  * **keys** (*List* *[* *str* *]*  *|* *None*)
* **Return type:**
...
* **Parameters:**
  * **key** (*str*) â The Redis key to set the expiration on.
  * **ttl** (*Optional* *[* *int* *]* *,* *optional*) â The time-to-live in seconds. If None,
...
* **Parameters:**
  * **prompt** (*str*) â The user prompt to cache.
  * **response** (*str*) â The LLM response to cache.
  * **vector** (*Optional* *[* *List* *[* *float* *]* *]* *,* *optional*) â The prompt vector to
    cache. Defaults to None, and the prompt vector is generated on
    demand.
  * **metadata** (*Optional* *[* *Dict* *[* *str* *,* *Any* *]* *]* *,* *optional*) â The optional metadata to cache
    alongside the prompt and response. Defaults to None.
  * **filters** (*Optional* *[* *Dict* *[* *str* *,* *Any* *]* *]*) â The optional tag to assign to the cache entry.
    Defaults to None.
  * **ttl** (*Optional* *[* *int* *]*) â The optional TTL override to use on this individual cache
...
* **Return type:**
  str
* **Raises:**
  * **ValueError** â If neither prompt nor vector is specified.
  * **ValueError** â if vector has incorrect dimensions.
  * **TypeError** â If provided metadata is not a dictionary.
...
* **Parameters:**
  **key** (*str*) â the key of the document to update using kwargs.
* **Raises:**
  * **ValueError if an incorrect mapping is provided as a kwarg.** â 
  * **TypeError if metadata is provided and not** **of** **type dict.** â 
* **Return type:**
  None
...
* **Parameters:**
  **distance_threshold** (*float*) â The semantic distance threshold for
  the cache.
* **Raises:**
  **ValueError** â If the threshold is not between 0 and 1.
* **Return type:**
...
* **Parameters:**
  **ttl** (*Optional* *[* *int* *]* *,* *optional*) â The optional time-to-live expiration
  for the cache, in seconds.
* **Raises:**
  **ValueError** â If the time-to-live value is not an integer.
* **Return type:**
  None
...
* **Parameters:**
  * **name** (*str*) â The name of the cache. Defaults to "embedcache".
  * **ttl** (*Optional* *[* *int* *]*) â The time-to-live for cached embeddings. Defaults to None.
  * **redis_client** (*Optional* *[* *SyncRedisClient* *]*) â Redis client instance. Defaults to None.
  * **redis_url** (*str*) â Redis URL for connection. Defaults to "redis://localhost:6379".
  * **connection_kwargs** (*Dict* *[* *str* *,* *Any* *]*) â Redis connection arguments. Defaults to {}.
  * **async_redis_client** (*Redis* *|* *RedisCluster* *|* *None*)
* **Raises:**
  **ValueError** â If vector dimensions are invalid
...
* **Parameters:**
  * **text** (*str*) â The text input that was embedded.
  * **model_name** (*str*) â The name of the embedding model.
* **Return type:**
  None
...
* **Parameters:**
  **key** (*str*) â The full Redis key for the embedding.
* **Return type:**
  None
...
* **Parameters:**
  * **texts** (*List* *[* *str* *]*) â List of text inputs that were embedded.
  * **model_name** (*str*) â The name of the embedding model.
* **Return type:**
  None
...
* **Parameters:**
  **keys** (*List* *[* *str* *]*) â List of Redis keys to remove.
* **Return type:**
  None
...
* **Parameters:**
  **keys** (*List* *[* *str* *]*) â List of Redis keys to check.
* **Returns:**
...
* **Parameters:**
  **keys** (*List* *[* *str* *]*) â List of Redis keys to retrieve.
* **Returns:**
...
- âtextâ: The text input that was embedded
- âmodel_nameâ: The name of the embedding model
- âembeddingâ: The embedding vector
- âmetadataâ: Optional metadata to store with the embedding

* **Parameters:**
  * **items** (*List* *[* *Dict* *[* *str* *,* *Any* *]* *]*) â List of dictionaries, each containing text, model_name, embedding, and optional metadata.
  * **ttl** (*int* *|* *None*) â Optional TTL override for these entries.
* **Returns:**
...
* **Parameters:**
  * **text** (*str*) â The text input that was embedded.
  * **model_name** (*str*) â The name of the embedding model.
  * **embedding** (*List* *[* *float* *]*) â The embedding vector to store.
  * **metadata** (*Optional* *[* *Dict* *[* *str* *,* *Any* *]* *]*) â Optional metadata to store with the embedding.
  * **ttl** (*Optional* *[* *int* *]*) â Optional TTL override for this specific entry.
* **Returns:**
