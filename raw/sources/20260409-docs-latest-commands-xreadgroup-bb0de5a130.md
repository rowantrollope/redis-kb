...
retrieved_utc: '2026-04-09T20:45:38.208174+00:00'
tags:
- official
- docs
- sitemap
fetched_url: https://redis.io/docs/latest/commands/xreadgroup/index.html.md
---
...
```json metadata
{
  ...
  "since": "5.0.0",
  "arity": -7,
  "key_specs": [{"RO":true,"access":true,"begin_search":{"spec":{"keyword":"STREAMS","startfrom":4},"type":"keyword"},"find_keys":{"spec":{"keystep":1,"lastkey":-1,"limit":2},"type":"range"}}],
  "tableOfContents": {"sections":[{"id":"consumer-groups-in-30-seconds","title":"Consumer groups in 30 seconds"},{"id":"differences-between-xread-and-xreadgroup","title":"Differences between XREAD and XREADGROUP"},{"children":[{"id":"reply-extension-for-claimed-entries","title":"Reply extension for claimed entries"},{"id":"ordering-guarantees","title":"Ordering guarantees"}],"id":"the-claim-option","title":"The CLAIM option"},{"id":"what-happens-when-a-message-is-delivered-to-a-consumer","title":"What happens when a message is delivered to a consumer?"},{"id":"usage-example","title":"Usage example"},{"id":"what-happens-when-a-pending-message-is-deleted","title":"What happens when a pending message is deleted?"},{"id":"redis-software-and-redis-cloud-compatibility","title":"Redis Software and Redis Cloud compatibility"},{"id":"return-information","title":"Return information"}]}
...
When `CLAIM min-idle-time` is used, additional information is provided for each pending entry retrieved (similar to the reply of [`XPENDING`]()). For each claimed pending entry, the reply includes:
...
Entries may be deleted from the stream due to trimming with [`XADD`]() or [`XTRIM`](),
or explicit calls to [`XDEL`](), [`XDELEX`](), or [`XACKDEL`]().

When an entry is trimmed with [`XADD`]() or [`XTRIM`]() and `DELREF` or `ACKED` are not specified,
deleted with [`XDEL`](), or deleted with [`XDELEX`]() or [`XACKDEL`]() 
