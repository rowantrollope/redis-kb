...
url: https://redis.io/docs/latest/integrate/nagios-with-redis-enterprise/
retrieved_utc: '2026-04-09T20:45:53.576797+00:00'
tags:
- official
- docs
- sitemap
fetched_url: https://redis.io/docs/latest/integrate/nagios-with-redis-enterprise/index.html.md
---
...
{
  "title": "Nagios with Redis Software",
  "description": "The Redis Software (RS)Â Nagios plugin enables you to monitor the status of RSÂ related objects and alerts. The RSÂ alerts can be related to the cluster, nodes, or databases.",
  "categories": ["docs","integrate","rs"],
  "group": "observability",
  "tableOfContents": {"sections":[{"children":[{"id":"step-1","title":"Step 1"},{"id":"step-2","title":"Step 2"},{"id":"step-3","title":"Step 3"}],"id":"configuring-the-nagios-plugin","title":"Configuring the Nagios plugin"}]}
...
specific Database Â­\> Configuration page.

All alert configurations (active / not active, setting thresholds, etc')
can only be done through the RSÂ UI, they cannot be configured in Nagios.
...
RSÂ Nagios plugin support API password retrieval from Gnome keyring,
...
1. pip install keyring Â­to install the package (See
    https://pip.pypa.io/en/stable/installing/ on how to install python
    pip if needed).
1. keyring set RS-Nagios Â­\<RS user email\> to set the password.
...
The RSÂ Nagios package includes two components:

- The plugin itself Â­- with suffix "rlec_nagios_plugin"
...
Copy the folder named "libexec"Â from the plugin folder and its contents
to "/usr/local/nagios/"
...
"Â­h" switch in order to retrieve their documentation and their expected
parameters.
...
1. Under the "/etc" folder:
    1. "rlec_resource.cfg " Â­ holds global variables definitions for
...
1. Under the "/rlec_local" folder:
    1. "cluster.cfg " Â­ holds configuration details at the cluster
        ...
        1. The first "define host" section defines a variable for the
            ...
            1. Update the "address" to the Cluster Name (FQDN) as
                ...
                the cluster.
            1. If you are configuring more than one RSÂ then when
    ...
    1. "contacts.cfg " Â­ holds configuration details who to send emails
    ...
    1. "databases.cfg" Â­ holds configuration details of the databases
        ...
        1. "_RLECID " should be the database's internal ID that can
            be retrieved from
            [`rladmin status`]() command output.
    1. "nodes.cfg " Â­ holds configuration details of the nodes in the
        ...
        1. "_RLECID " should be the node's internal ID that can be
            retrieved
            fromÂ [`rladmin status`]() command output.
    1. Under the "/rlec_obj" folder:
        1. "rlec_cmd.cfg" Â­ holds configuration details of how to
        ...
        1. "rlec_templates.cfg" holds general RSÂ Nagios definitions.
