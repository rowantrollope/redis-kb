---
title: Time series
url: https://redis.io/docs/latest/operate/oss_and_stack/stack-with-enterprise/timeseries/
retrieved_utc: '2026-04-09T20:45:58.257396+00:00'
tags:
- official
- docs
- sitemap
fetched_url: https://redis.io/docs/latest/operate/oss_and_stack/stack-with-enterprise/timeseries/index.html.md
---
...
```json metadata
{
  ...
  "tableOfContents": {"sections":[{"id":"features","title":"Features"},{"id":"memory-model","title":"Memory model"},{"children":[{"id":"downsamplingcompaction","title":"Downsampling/compaction"},{"id":"secondary-indexing","title":"Secondary indexing"},{"id":"aggregation-at-read-time","title":"Aggregation at read time"},{"id":"integrations","title":"Integrations"}],"id":"time-series-capabilities","title":"Time series capabilities"},{"children":[{"id":"data-modeling-approaches","title":"Data modeling approaches"},{"id":"data-ingestion","title":"Data ingestion"},{"id":"read-performance","title":"Read performance"},{"id":"memory-utilization","title":"Memory utilization"}],"id":"time-series-modeling-approaches-with-redis","title":"Time series modeling approaches with Redis"},{"id":"more-info","title":"More info"}]}
...
When using Redisâ core data structures, you can only retrieve a time series by knowing the exact key holding the time series. Unfortunately, for many time series use cases (such as root cause analysis or monitoring), your application wonât know the exact key itâs looking for. These use cases typically want to query a set of time series that relate to each other in a couple of dimensions to extract the insight you need. You could create your own secondary index with core Redis data structures to help with this, but it would come with a high development cost and require you to manage edge cases to make sure the index is correct.

Redis does this indexing for you based on `field value` pairs called [labels](). You can add labels to each time series and use them to [filter]() at query time.

Hereâs an example of creating a time series with two labels (sensor_id and area_id are the fields with values 2 and 32 respectively) and a retention window of 60,000 milliseconds:
...
When you need to query a time series, itâs cumbersome to stream all raw data points if youâre only interested in, say, an average over a given time interval. Time series only transfer the minimum required data to ensure lowest latency.
...
For sorted sets, we modeled the data in two different ways. For "Sorted set per device", we concatenated the metrics and separated them out by colons, e.g. `"<timestamp>:<metric1>:<metric2>: â¦ :<metric10>"`.



Of course, this consumes less memory but needs more CPU cycles to get the correct metric at read time. It also implies that changing the number of metrics per device isnât straightforward, which is why we also benchmarked a second sorted set approach. In "Sorted set per metric," we kept each metric in its own sorted set and had 10 sorted sets per device. We logged values in the format `"<timestamp>:<metric>"`.
...
Our benchmark did not utilize the out-of-the-box secondary indexing capabilities of time series. Redis keeps a partial secondary index in each shard, and since the index inherits the same hash-slot of the key it indexes, it is always hosted on the same shard. This approach would make the setup for native data structures even more complex to model, so for the sake of simplicity, we decided not to include it in our benchmarks. Additionally, while Redis Software can use the [proxy](https://redis.com/redis-enterprise/technology/redis-enterprise-cluster-architecture/) to fan out requests for commands like [TS.MGET]() and [TS.MRANGE]() to all the shards and aggregate the results, we chose not to exploit this advantage in the benchmark either.

### Data ingestion

For the data ingestion part of our benchmark, we compared the four approaches by measuring how many devicesâ data we could ingest per second. Our client side had 8 worker threads with 50 connections each, and a pipeline of 50 commands per request.
