...
url: https://redis.io/docs/latest/operate/rs/databases/active-active/disaster-recovery/
retrieved_utc: '2026-04-09T20:45:57.851712+00:00'
tags:
- official
- docs
- sitemap
fetched_url: https://redis.io/docs/latest/operate/rs/databases/active-active/disaster-recovery/index.html.md
---
...
```json metadata
{
  ...
  "tableOfContents": {"sections":[{"id":"disaster-recovery-strategies","title":"Disaster recovery strategies"},{"id":"considerations-for-disaster-recovery","title":"Considerations for disaster recovery"},{"children":[{"id":"lag-aware-database-availability-requests","title":"Lag-aware database availability requests"},{"id":"redis-connection-health-checks","title":"Redis connection health checks"},{"id":"health-check-comparison","title":"Health check comparison"}],"id":"detect-failures-with-health-checks","title":"Detect failures with health checks"}]}
...
- [Network-based](): Global traffic managers and load balancers handle routing at the network layer, requiring no application changes.

- [Proxy-based](): Software proxies handle detection and routing logic.

- [Client library-based](): Redis client libraries with built-in failover logic.
...
- Does the Active-Active database use DNS, the [OSS Cluster API](), or the [discovery service]()?
...
See [Lag-aware database availability requests]() for more information.
...
The [`PING`]() command checks the following:
...
For example, you can check the keyspace with write operations such as using the [`SET`]() command to write arbitrary data. This check verifies that database shards are available and writable.
