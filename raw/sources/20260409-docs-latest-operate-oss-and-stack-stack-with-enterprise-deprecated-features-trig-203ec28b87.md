...
url: https://redis.io/docs/latest/operate/oss_and_stack/stack-with-enterprise/deprecated-features/triggers-and-functions/concepts/cluster_support/
retrieved_utc: '2026-04-09T20:45:52.813200+00:00'
tags:
- official
- docs
- sitemap
fetched_url: https://redis.io/docs/latest/operate/oss_and_stack/stack-with-enterprise/deprecated-features/triggers-and-functions/concepts/cluster_support/index.html.md
---
...
```json metadata
{
  ...
  "tableOfContents": {"sections":[{"id":"arguments-and-results-serialization","title":"Arguments and results serialization"},{"id":"execution-timeout","title":"Execution timeout"},{"id":"remote-function-limitations","title":"Remote function limitations"}]}
...
`redis.registerClusterFunction` is passed the remote function name, which will be used later to call the remote function, and the remote function code. The remote function must be a Coroutine (async function) and it is executed in the background on the remote shard. For more information about async function, please refer to [sync and async]() page.
...
Remote functions will not be permitted to run forever and will timeout. The timeout period can be configured using [remote-task-default-timeout]().
...
All the limitations listed on [coroutines]() also apply to remote functions. Remote function also come with some extra limitations:
