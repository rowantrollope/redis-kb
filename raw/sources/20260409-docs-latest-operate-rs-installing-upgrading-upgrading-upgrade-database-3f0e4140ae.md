...
retrieved_utc: '2026-04-09T20:45:58.281264+00:00'
tags:
- official
- docs
- sitemap
fetched_url: https://redis.io/docs/latest/operate/rs/installing-upgrading/upgrading/upgrade-database/index.html.md
---
...
```json metadata
{
  ...
  "tableOfContents": {"sections":[{"id":"default-redis-database-versions-default-db-versions","title":"Default Redis database versions {#default-db-versions}"},{"id":"upgrade-prerequisites","title":"Upgrade prerequisites"},{"id":"upgrade-database","title":"Upgrade database"}]}
...
1. Upgrade the database using [`rladmin upgrade db`](). During the upgrade process, the database will restart without losing any data. Use the `preserve_roles` option to keep the database's current state, including primary shard placement, and prevent the cluster from becoming unbalanced.
...
1. Use [`rladmin status databases extra all`]() to display a list of the databases in your cluster and their current Redis database compatibility version. Verify that the Redis version is set to the expected value.
