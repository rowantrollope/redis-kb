...
url: https://redis.io/docs/latest/operate/rc/api/examples/generate-cost-report/
retrieved_utc: '2026-04-09T20:45:58.542762+00:00'
tags:
- official
- docs
- sitemap
fetched_url: https://redis.io/docs/latest/operate/rc/api/examples/generate-cost-report/index.html.md
---
...
```json metadata
{
  ...
  "tableOfContents": {"sections":[{"id":"generate-a-cost-report","title":"Generate a cost report"},{"id":"get-cost-report-status","title":"Get cost report status"},{"children":[{"id":"cost-report-fields","title":"Cost report fields"}],"id":"download-cost-report","title":"Download cost report"}]}
...
| `tags` | Array of key-value pairs | Filters the report to only include databases with the specified [tags](). Both `key` and `value` are required for each tag. |
...
The response body is a [task object]() that contains the `taskId` for the task that generates the cost report:
...
To get the status of the cost report generation, use [`GET /tasks/{taskId}`]() with the `taskId` from the previous step.
...
To get the cost report, use [`GET /cost-report/{costReportId}`]() with the `costReportId` from the previous step.
