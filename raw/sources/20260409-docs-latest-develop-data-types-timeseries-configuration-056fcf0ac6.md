...
url: https://redis.io/docs/latest/develop/data-types/timeseries/configuration/
retrieved_utc: '2026-04-09T20:45:52.564227+00:00'
tags:
- official
- docs
- sitemap
fetched_url: https://redis.io/docs/latest/develop/data-types/timeseries/configuration/index.html.md
---
...
```json metadata
{
  ...
  "tableOfContents": {"sections":[{"id":"redis-open-source-set-configuration-parameters","title":"Redis Open Source - set configuration parameters"},{"children":[{"id":"chunksizebytes-ts-chunk-size-bytes","title":"CHUNKSIZEBYTES / ts-chunk-size-bytes"},{"id":"compactionpolicy-ts-compaction-policy","title":"COMPACTION_POLICY / ts-compaction-policy"},{"id":"duplicatepolicy-ts-duplicate-policy","title":"DUPLICATE_POLICY / ts-duplicate-policy"},{"id":"retentionpolicy-ts-retention-policy","title":"RETENTION_POLICY / ts-retention-policy"},{"id":"encoding-ts-encoding","title":"ENCODING / ts-encoding"},{"id":"ignoremaxtimediff-ts-ignore-max-time-diff-and-ignoremaxvaldiff-ts-ignore-max-val-diff","title":"IGNOREMAXTIMEDIFF / ts-ignore-max-time-diff and IGNOREMAXVALDIFF / ts-ignore-max-val-diff"},{"id":"numthreads-ts-num-threads","title":"NUM_THREADS / ts-num-threads"},{"id":"ossglobalpassword","title":"OSSGLOBALPASSWORD"}],"id":"time-series-configuration-parameters","title":"Time series configuration parameters"}]}
...
1. Key-level policy, as set with [`TS.CREATE`]()'s and [`TS.ALTER`]()'s `CHUNK_SIZE` optional argument.
...
Default compaction rules for newly created keys with [`TS.ADD`](), [`TS.INCRBY`](), and  [`TS.DECRBY`]().
...
The default policy for handling insertion ([`TS.ADD`]() and [`TS.MADD`]()) of multiple samples with identical timestamps, with one of the following values:
...
1. Key-level policy, as set with [`TS.CREATE`]()'s and [`TS.ALTER`]()'s `DUPLICATE_POLICY` optional argument.
...
The retention period is the maximum age of samples compared to the highest reported timestamp, per key. Samples are expired based solely on the difference between their timestamps and the timestamps passed to subsequent [`TS.ADD`](), [`TS.MADD`](), [`TS.INCRBY`](), and [`TS.DECRBY`]() calls.
...
1. The sample is added in-order (`timestamp â¥ max_timestamp`).
...
The maximum number of per-shard threads for cross-key queries when using cluster mode ([`TS.MRANGE`](), [`TS.MREVRANGE`](), [`TS.MGET`](), and [`TS.QUERYINDEX`]()). The value must be equal to or greater than `1`. Note that increasing this value may either increase or decrease the performance!
