...
retrieved_utc: '2026-04-09T20:45:56.237630+00:00'
tags:
- official
- docs
- sitemap
fetched_url: https://redis.io/docs/latest/develop/clients/client-side-caching/index.html.md
---
...
```json metadata
{
  ...
  "topics": ["client-side-caching","performance"],
  "relatedPages": ["/develop/clients/redis-py/connect#connect-using-client-side-caching","/develop/clients/nodejs/connect#connect-using-client-side-caching","/develop/clients/jedis/connect#connect-using-client-side-caching"],
  "scope": "overview",
  "tableOfContents": {"sections":[{"id":"updating-the-cache-when-the-data-changes-tracking","title":"Updating the cache when the data changes {#tracking}"},{"id":"which-client-libraries-support-client-side-caching","title":"Which client libraries support client-side caching?"},{"id":"which-commands-can-cache-data","title":"Which commands can cache data?"},{"id":"what-data-gets-cached-for-a-command","title":"What data gets cached for a command?"},{"id":"usage-recommendations","title":"Usage recommendations"}]}
...
as with [`STRLEN`](). When any client
...
-   Non-deterministic commands such as [`HRANDFIELD`](),
    [`HSCAN`](),
    and [`ZRANDMEMBER`](). By design, these commands
    give different results each time they are called.
-   Redis Search commands (with the `FT.*` prefix), such as
    [`FT.SEARCH`]().

You can use the [`MONITOR`]() command to
...
-   The whole string retrieved by [`GET`]()
    is added to the cache. Parts of the same string retrieved by
    [`SUBSTR`]() are calculated on the
...
-   Ranges from [lists](),
    ...
    subsets returned by [`SINTER`]() and
    [`SDIFF`]() create separate cache entries.
-   For multi-key read commands such as [`MGET`](),
...
-   Boolean or numeric values calculated from data types (for example 
    [`SISMEMBER`]()) and
    [`LLEN`]() are cached separately from the
